...
class Generator_T1(nn.Module):
    """
    Generator network for recreating the missing image in the sequence of T1,T2,PDFF maps etc.
    """
    def __init__(self,nC,xDim=128,yDim=128,outC=1):
        super(Generator_T1,self).__init__()
        self.nC = nC
...
class Generator_noBN(nn.Module):
    ...
    def __init__(self,nC,xDim=128,yDim=128,outC=1):
        ...
        self.inConv = DoubleConv_noBN(self.nC,64)
        self.down1 = DownConv_noBN(64,128)
        self.down2 = DownConv_noBN(128,256)
        self.down3 = DownConv_noBN(256,512)
        self.down4 = DownConv_noBN(512,512)
        self.up1 = UpConv_noBN(1024,256)
        self.up2 = UpConv_noBN(512,128)
        self.up3 = UpConv_noBN(256,64)
        self.up4 = UpConv_noBN(128,64)
        self.outConv1 = nn.Conv2d(64,32,kernel_size=1)
        self.outConv2 = nn.Conv2d(32,16,kernel_size=1)
        self.outConv3 = nn.Conv2d(16,outC,kernel_size=1)
...
class Discriminator_noBN(nn.Module):
    ...
    def __init__(self,nC,xDim=128,yDim=128):
        ...
        self.down2 = DownConv_noBN(128,256)
        self.down3 = DownConv_noBN(256,512)
        self.down4 = DownConv_noBN(512,1024)
...
class DoubleConv_noBN(nn.Module):
    ...
    """
    def __init__(self,inC,outC):
        super(DoubleConv_noBN,self).__init__()
...
class DownConv_noBN(nn.Module):
    """
    Downsample and Double Conv
    """
    def __init__(self,inC,outC):
        super(DownConv_noBN,self).__init__()

        self.down_conv = nn.Sequential(nn.MaxPool2d(2),DoubleConv_noBN(inC,outC))

    def forward(self, x):
        return self.down_conv(x)

class UpConv_noBN(nn.Module):
    """
    Upsample, concatenation and Double Conv
    """
    def __init__(self,inC,outC):
        super(UpConv_noBN,self).__init__()

        self.up = nn.Upsample(scale_factor=2,mode='bilinear',align_corners=True)

        self.conv = DoubleConv_noBN(inC,outC)
